...
export function parseSearchPage(
  ...
  const totalText = $('[data-component-type="s-result-info-bar"] .a-size-medium').text().trim()
  const totalMatch = totalText.match(/[\d,]+/)
  const total_results = totalMatch ? parseInt(totalMatch[0].replace(/,/g, '')) : null
  ...
  $('[data-component-type="s-search-result"][data-asin]').each((idx, el) => {
    ...
    const link = href ? `https://www.${domain}${href.split('?')[0]}` : `https://www.${domain}/dp/${asin}`
